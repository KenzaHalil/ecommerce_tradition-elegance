...
class DevOrderService:
    ...
    def create_order(self, user_id, items, total_cents, status="PENDING"):
        ...
        order = {"id": oid, "user_id": user_id, "items": items, "total_cents": int(total_cents or 0), "status": status, "created_at": now}
...
def _wrap_order_service(raw):
    """
    Retourne un objet exposant create_order(...), get_user_orders(...) et set_status(...)
    - Si raw a déjà create_order on le renvoie.
    - Sinon on essaie quelques noms alternatifs, sinon on renvoie DevOrderService() en fallback.
    """
    if not raw:
        return DevOrderService()
    if hasattr(raw, "create_order"):
        return raw
    alt_names = ("create", "place_order", "placeOrder", "createOrder", "make_order", "new_order")
        ...
        if hasattr(raw, name):
            ...
            class OrderAdapter:
                def __init__(self, fn, raw):
                    ...
                    self._raw = raw
                def create_order(self, user_id, items, total_cents, status="PENDING"):
                    return self._fn(user_id, items, total_cents)
                def get_user_orders(self, user_id):
                    return getattr(self._raw, "get_user_orders", lambda uid: [])(user_id)
                def get_order(self, order_id):
                    return getattr(self._raw, "get_order", lambda oid: None)(order_id)
                def set_status(self, order_id, status):
                    if hasattr(self._raw, "set_status"):
                        return self._raw.set_status(order_id, status)
                    return None
            return OrderAdapter(fn, raw)
    return DevOrderService()
