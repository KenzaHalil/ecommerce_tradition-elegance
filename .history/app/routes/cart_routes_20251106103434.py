...
def api_update_item(product_id):
    """
    Met à jour la quantité en session (ou via service si nécessaire) et renvoie
    le subtotal de l'article et le total du panier (en cents) en JSON.
    """
    try:
        data = request.get_json(silent=True) or {}
        qty = int(data.get("qty", request.form.get("qty", 0)))
    except Exception:
        qty = 0

    # Mise à jour session cart (fallback fiable)
    ...
    # Calculer subtotal pour l'article et total du panier (en cents)
    def _get_price_cents(pid):
        try:
            p = Product.query.get(pid)
            ...
            try:
                p = Product.get(pid)
                return int(p.get("price_cents", 0) if isinstance(p, dict) else 0)
            except Exception:
                return 0

    price_cents = _get_price_cents(product_id)
    subtotal = price_cents * (qty or 0)

    total = 0
    for pid, q in cart.items():
        total += _get_price_cents(pid) * int(q or 0)
