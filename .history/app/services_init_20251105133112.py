...
class DevOrderService:
    """Fallback order service that uses the project's SQLAlchemy models so orders
    are visible depuis les routes qui utilisent Order / OrderItem."""
    def __init__(self):
        pass
    ...
    def create_order(self, user_id, items, total_cents, status="PENDING"):
        ...
        order = Order(user_id=uid, status=status, total_cents=int(total_cents or 0), created_at=datetime.utcnow())
        db.session.add(order)
        db.session.flush()  # get order.id
        for it in items or []:
            if isinstance(it, dict):
                pid = it.get("product_id") or it.get("id") or ""
                qty = int(it.get("quantity", 1))
                price = int(it.get("price_cents", 0))
            else:
                pid = getattr(it, "product_id", None) or getattr(it, "id", "")
                qty = int(getattr(it, "quantity", 1))
                price = int(getattr(it, "price_cents", 0) or 0)
            oi = OrderItem(order_id=order.id, product_id=str(pid), quantity=qty, price_cents=price)
            db.session.add(oi)
        db.session.commit()
        return {"id": order.id, "user_id": order.user_id, "items": items, "total_cents": order.total_cents, "status": order.status, "created_at": order.created_at}

    def get_user_orders(self, user_id):
        from app.models import Order, OrderItem
        uid = int(user_id) if isinstance(user_id, (str, int)) and str(user_id).isdigit() else user_id
        qs = Order.query.filter_by(user_id=uid).order_by(Order.created_at.desc()).all()
        out = []
        for o in qs:
            its = []
            for it in getattr(o, "items", []) or []:
                its.append({
                    ...
                    "subtotal_cents": (it.price_cents or 0) * (it.quantity or 0)
                })
            out.append({
                "id": o.id,
                "user_id": o.user_id,
    ...
    def get_order(self, order_id):
        ...
        its = []
        for it in getattr(o, "items", []) or []:
            its.append({"product_id": it.product_id, "quantity": it.quantity, "price_cents": it.price_cents, "subtotal_cents": (it.price_cents or 0)*(it.quantity or 0)})
        return {"id": o.id, "user_id": o.user_id, "items": its, "total_cents": o.total_cents, "status": o.status, "created_at": o.created_at}
