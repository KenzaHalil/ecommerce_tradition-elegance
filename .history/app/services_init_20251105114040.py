...
class AuthService:
    ...
    def register(self, email: str, password: str, first_name: str = "", last_name: str = "", is_admin: bool = False):
        ...
        cur.execute(
            "INSERT INTO user (email, password_hash, first_name, last_name, is_admin, created_at) VALUES (?, ?, ?, ?, ?, ?)",
            (email, pw_hash, first_name or None, last_name or None, 1 if is_admin else 0, now)
        )
        conn.commit()
        uid = cur.lastrowid
        conn.close()
...
def init_services(app):
    ...
    services = {
        "auth": auth,
        "products": products,
        # wrap the domain CartService with the adapter expected by routes
