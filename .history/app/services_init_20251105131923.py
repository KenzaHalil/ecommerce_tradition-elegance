...
class DevOrderService:
    ...
    def create_order(self, user_id, items, total_cents, status="PENDING"):
        """Persist order + items into SQLite and keep in-memory copy."""
        import uuid
        from datetime import datetime
        oid = str(uuid.uuid4())
        uid = str(user_id)
        ...
        cur.execute(
            ...
            (oid, uid, int(total_cents or 0), status, now)
        )
        # insert items (expect items as list of dicts with product_id, quantity, price_cents)
        for it in items or []:
            # normalize shape
            pid = str(it.get("product_id") if isinstance(it, dict) else getattr(it, "product_id", None) or "")
            qty = int(it.get("quantity", 1) if isinstance(it, dict) else getattr(it, "quantity", 1))
            price = int(it.get("price_cents", 0) if isinstance(it, dict) else getattr(it, "price_cents", 0) or 0)
            cur.execute(
        ...
        conn.commit()
        conn.close()
        order = {"id": oid, "user_id": uid, "items": items, "total_cents": int(total_cents or 0), "status": status, "created_at": now}
        self._orders.append(order)
        return order

    def get_user_orders(self, user_id):
        """Read persisted orders + items from DB for given user_id."""
        uid = str(user_id)
        conn = _connect()
        cur = conn.cursor()
        cur.execute("SELECT id, total_cents, status, created_at FROM orders WHERE user_id = ? ORDER BY created_at DESC", (uid,))
        rows = cur.fetchall()
        out = []
        for r in rows:
            oid, total_cents, status, created_at = r
            cur.execute("SELECT product_id, quantity, price_cents FROM order_items WHERE order_id = ?", (oid,))
            its = []
    ...
    def get_order(self, order_id):
        ...
        if not r:
            conn.close()
            return None
        oid, uid, total_cents, status, created_at = r
        cur.execute("SELECT product_id, quantity, price_cents FROM order_items WHERE order_id = ?", (oid,))
        its = []
        for pit, qty, price in cur.fetchall():
            its.append({
        ...
        conn.close()
        return {"id": oid, "user_id": uid, "items": its, "total_cents": int(total_cents or 0), "status": status, "created_at": created_at}
